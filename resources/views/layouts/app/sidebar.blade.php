...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        ...
        <nav class="lg:hidden fixed bottom-0 inset-x-0 z-[45] bg-zinc-950/95 backdrop-blur border-t border-zinc-800 flex items-stretch h-16">
            ...
            <a href="{{ route('shop.index') }}" wire:navigate class="flex-1 flex flex-col items-center justify-center gap-1">
                <svg class="w-5 h-5 {{ request()->routeIs('shop.*') ? 'text-zinc-100' : 'text-zinc-600' }}" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                </svg>
                <span class="text-[10px] {{ request()->routeIs('shop.*') ? 'text-zinc-100 font-medium' : 'text-zinc-600' }}">Shop</span>
            </a>
